<template>
    <div class="mx-5 p-2 border border-dark">
        <h3 class="bg-info text-white tex-center p-2">{{ operation }}</h3>
        <div class="container-fluid">
            <div class="row">
                ...
                <div class="col h3" v-bordersize="total > 25">= {{ total }}</div>
            </div>
        </div>
    </div>
</template>
    ...
    computed: {
        ...
        total() {
            switch (this.operation.toLowerCase()) {
                ...
                case "substract":
                    return this.$calc.substract(this.first, this.second);
                case "multiply":
